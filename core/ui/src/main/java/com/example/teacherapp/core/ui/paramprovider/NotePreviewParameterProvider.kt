...
class NotesPreviewParameterProvider : PreviewParameterProvider<List<Note>> {
    override val values: Sequence<List<Note>> = sequenceOf(listOf(*notes))
}

class NotePreviewParameterProvider : PreviewParameterProvider<Note> {
    override val values: Sequence<Note> = sequenceOf(*notes)
}

private val notes = makeNotes(
...
private fun makeNotes(vararg data: Pair<String, String>): Array<Note> =
...
private fun makeNote(id: Long, title: String, text: String): Note {
    ...
    return Note(
        ...
        title = title,
        text = text,
        priority = priorities[id.toInt() % priorities.size],
    )
}